...
export namespace NibroPF2E {
  export interface PF2EInterface {}

  export class PF2EImplementation implements PF2EInterface {
    constructor() {
      ...
      NibroCore.NibroCore.registerMacro({
        ...
        advancedAction: (ctx: NibroCore.Context) => {
          const names = TokenUtils.TokenUtils.setSelected(ctx, {
            bar1_value: {
      ...
      NibroCore.NibroCore.registerMacro({
        ...
        isVisibleToAll: false,
        advancedArgs: [
          "?{Vision|Standard Vision|Low-Light Vision|Dark Vision}",
        ],
      ...
      NibroCore.NibroCore.registerMacro({
        ...
        isVisibleToAll: true,
        advancedAction: (ctx) => {
          TokenUtils.TokenUtils.setSelected(ctx, {
            //Toggle Lights
      ...
      NibroCore.NibroCore.registerMacro({
        ...
        advancedAction: (ctx, args) => {
          const characters = PF2EImplementation.getSelectedCharacters(ctx);
          switch (args[0]) {
            case "Perception": {
              NibroCore.NibroCore.reply(
                ctx,
                `
                                  &{template:rolls}
                                  {{header=^{perception}}}
    ...
    private static getSelectedCharacters(ctx: NibroCore.Context): Character[] {
  ...
  export let PF2ETools: PF2EInterface = new PF2EImplementation();
}
